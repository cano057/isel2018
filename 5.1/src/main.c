...
int button_pressed(fsm_t* this) {
  if(GPIO_INPUT_GET(GPIO_D8)) {
  // if((GPIO_INPUT_GET(GPIO_D8)) || (!GPIO_INPUT_GET(GPIO_D3))) {
    xLastButtonPressed = xTaskGetTickCount();
...
int time_up(fsm_t* this) {
  return ( ( xTaskGetTickCount() - xLastButtonPressed ) >= 500 );
}
void led_on(fsm_t* this) {
  GPIO_OUTPUT_SET(GPIO_D4,0);
}
...
static fsm_trans_t interruptor[] = {
  ...
  { LED_ON, button_pressed, LED_ON, led_on },
  { LED_ON, time_up, LED_OFF, led_off},
  {-1, NULL, -1, NULL},
};
